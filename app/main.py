...
Vercel:      auto-detected at app/main.py (exports `app`)

Unlike the original version, this does NOT train models at startup.
Models are pretrained offline (scripts/train_models.py) and pickled into
app/models/*.joblib, so a cold start just has to unpickle a few files —
fast, deterministic, and doesn't burn function execution time retraining
on every container spin-up.
"""
...
from app.news_service import CATEGORIES, NewsService

manager = ModelManager()
news_service = None  # initialized after models are loaded


@asynccontextmanager
async def lifespan(app: FastAPI):
    global news_service
    print("\nFakeShield starting up — loading pretrained models...")
    manager.load_pretrained()
    # First-ever lemmatize() call lazily loads the full WordNet corpus into
    # memory (~4s). Pay that cost once here during startup instead of on the
    # first real user request.
    manager.predict("warming up the pipeline", "ensemble")
    manager._stats = {"total": 0, "real": 0, "fake": 0}  # don't count the warm-up
    news_service = NewsService(manager)
    print("Models ready. NewsAPI key loaded:", bool(os.getenv("NEWS_API_KEY")))
    yield


app = FastAPI(title="FakeShield API", version="2.0.0", lifespan=lifespan)
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

FRONTEND = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "public", "index.html")
